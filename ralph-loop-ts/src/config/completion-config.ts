import type { CompletionConfig, CommandConfig } from "./config-types.ts";
...
export function parseCompletionConfigToml(tomlText: string): CompletionConfig {
  const parsed = Bun.TOML.parse(tomlText) as unknown;
  const root = assertObject(parsed, "completion config must be a TOML table");

  return {
    onLoopCompleteClaim: parseCommandArray(root.on_loop_complete_claim, "on_loop_complete_claim"),
  };
}
